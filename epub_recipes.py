...
def chunks(path):
    """Yield (href, title, text) units for the LLM pass.
    ...
    for href in spine:
        ...
        if not cuts:
            text = strip_tags(raw)
            if text:
                yield href, doc_titles.get(href, ""), text
            continue

        head = strip_tags(raw[:cuts[0][0]])
        if head:
            yield href, doc_titles.get(href, ""), head
        for i, (start, title) in enumerate(cuts):
            end = cuts[i + 1][0] if i + 1 < len(cuts) else len(raw)
            text = strip_tags(raw[start:end])
            if text:
                yield href, title, text
